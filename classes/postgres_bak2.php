 ...
 */
class postgres_bak2 extends library
{
	//
	// Properties
	//
	...
	public function sql()
	{
		if (!$this->conn)
		{
			$this->debug1(__FILE__, __FUNCTION__, __LINE__, "Not connected to database.");
			$this->last_return = false;
			return false;
		}
		...
		$this->debug1(__FILE__, __FUNCTION__, __LINE__, "num_list = %d", $num_list);
		$this->debug_r1(__FILE__, __FUNCTION__, __LINE__, $arg_list);
		...
		$this->debug1(__FILE__, __FUNCTION__, __LINE__, "params array");
		...
		{
			try {
				$this->debug2(__FILE__, __FUNCTION__, __LINE__, "Running: pg_query");
				$this->result_set = pg_query($this->conn, $sql);
			}
			catch(exception $e)
			{
				$this->debug1(__FILE__, __FUNCTION__, __LINE__, "pg_query() exception: %s", $e->getMessage());
			}
		}
		else
		{
			try {
				$this->result_set = pg_prepare($this->conn, $sql, $params);

				if (!$this->result_set )
				{
					$this->debug1(__FILE__, __FUNCTION__, __LINE__, "pg_prepare failed!");
					$this->last_return = false;
					return false;
				}
				...
				if (!$this->result_set )
				{
					$this->debug1(__FILE__, __FUNCTION__, __LINE__, "pg_execute failed!");
					$this->last_return = false;
					return false;
				}
			}
			...
			{
				$this->debug1(__FILE__, __FUNCTION__, __LINE__, "pg_execute() exception: %s", $e->getMessage());
			}
		}

		$this->debug_sql1(__FILE__, __FUNCTION__, __LINE__, "%s", $sql);

		//$this->rows_affected = pg_num_rows($rc);
		...
		$this->debug1(__FILE__, __FUNCTION__, __LINE__, "is_select_statement = %s", $this->is_select_statement == true ? "TRUE" : "FALSE");
